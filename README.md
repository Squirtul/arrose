...
## Webpage:
- The Pi will automatically host a webpage locally, on the port `8080`. Enter the URL `[Pi's IP]:8080` to visit the webpage.
- A brief log of the time, selected airport and number of aircraft shown at each refresh is displayed on the webpage.
- You can use the menu and `set` button at the top to set a new airport, which will be shown on the Pi similarly to ssh-ing in.
